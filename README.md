...
| **UDP** | 开始/停止监听（真建 socket）、收发、支持广播、目标地址与端口可改 |
| **TCP** | 服务端模式：开始/停止监听、接受 1 个客户端、收发（自由输入，不是写死的演示载荷） |
| **HTTP** | 起一个 httpd，记录进来的请求（方法 / URI / query / 关键头 / 请求体） |
| **MQTT** | 客户端：服务器 / 订阅主题 / **发布主题** / QoS 都可改，发布自由输入 |
...
- 桌面模拟器只有 UI，没有真实网络/串口/USB 行为
- TCP 只有服务端模式；HTTP 只有服务端模式（无客户端主动请求功能）
- MQTT 暂不支持 TLS / 用户名密码认证
- USB 转串口目前固定打开接口 0/2/4/6 对应的 1~4 通道，未做更多通道
